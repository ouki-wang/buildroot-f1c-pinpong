...
from pinpong.boards.f1c import resources as F1C_RES
from pinpong.boards.nezha import resources as NEZHA_RES
from pinpong.boards.jh7100 import resources as JH7100_RES
from pinpong.boards.rpi import resources as RPI_RES

def get_res(boardname):
  BOARDS_RES=[F1C_RES, NEZHA_RES, JH7100_RES, RPI_RES]
  for res in BOARDS_RES:
    if boardname == res["name"]:
      return res
  return None
...
class SYSFSPin:
  def __init__(self, board=None, pin=None, mode=None):
    ...
    if not os.path.exists('/sys/class/gpio/gpio'+str(self.pin)):
      ...
      if not os.path.exists('/sys/class/gpio/gpio'+str(self.pin)):
        raise ValueError("invalid pin num  %d"%self.pin)
...
class Pin:
  ...
  def __init__(self, board=None, pin=None, mode=None):
    ...
    elif self.board.res != None:#其他Linux开发板
      self.obj = SYSFSPin(board, self.board.pinmap[pin], mode)
    else:
...
class SYSFSPWM:
  def __init__(self, board, pin_obj):
    ...
    self.io = str(self.board.res.pwms[self.pin_obj.pin]["io"])
    self.channel = str(self.board.res.pwms[self.pin_obj.pin]["channel"])
...
class PWM:
  def __init__(self, board=None, pin_obj=None):
    ...
    if self.board.boardname == "RPI":#RPi
      ...
      if self.board.res[self.pin_obj.pin] in self.res["pwms"].keys():#其他Linux开发板
        self.obj = SYSFSPWM(board, self.pin_obj)
      else:
        raise ValueError("invalid pwm pin ",self.pin_obj.pin)
    else:
...
class SPI:
  ...
  def __init__(self, board=None, bus_num=0, device_num=0, sck=None, mosi=None, miso=None, cs=None, baudrate=100000, polarity=0, phase=0, bits=8):
    ...
    if self.board.res:  #RPiSPI
        #print("--------1111---bus_num=%d,device_num=%d"%(bus_num,device_num))
        self.board.spi[bus_num][device_num] = SYSSPI(bus_num, device_num)
...
class I2C:
  def __init__(self, board=None, bus_num=0):
    ...
    if self.board.res and bus_num not in self.board.res['i2c']:
      bus_num = self.board.res['i2c_fixed']
    if self.board.i2c[bus_num] == None:
      self.bus_num = bus_num
      if self.board.res != None:
...
class EVENTIRRecv:
  def __init__(self, board, pin_obj, callback):
    ...
    node = board.res['inputs'][self.pin_obj.pin]
...
class Tone:
  def __init__(self, board=None, pin_obj=None):
    ...
    self.board = board
    if self.board.res:
      self.obj = SYSFSTone(board, pin_obj)
    else:
...
class Servo:
  def __init__(self, board=None, pin_obj=None):
    ...
    if self.board.boardname == "RPI":
      ...
      if self.board.res[self.pin_obj.pin] in self.res["pwms"].keys():#其他Linux开发板
        self.obj = SYSFSServo(board, self.pin_obj)
    else:
...
class Board:
  def __init__(self, boardname="", port=None):
    ...
    self.res = get_res(self.boardname)
    if self.res:
      self.pinmap=self.res["pinmap"]
    if gboard == None:
      gboard = self
  ...
  def begin(self):
    ...
    if self.res or self.boardname in ["WIN"]:
      self.connected = True
      return self
    if self.connected:
      return
